...
// var base_url = "https://16630427.000webhostapp.com/";

var getUrl = window.location.href;
var url = new URL(getUrl);
var getCari = url.searchParams.get("cari");

// fungsi deskripsi dari encryptext menjadi plaintext
let deskripsi = function (encryptext) {
    if (!Number.isNaN(encryptext)) {
        encryptext = encryptext.toString();
    }

    let chars = [];
    for (index in encryptext) {
        let code = encryptext[index].charCodeAt(0);
        code = code - 3;
        chars.push(String.fromCharCode(code));
        // console.log(encryptext[index] + " => " + chars[index]);
    }
...
app.controller('barangController', function ($scope, $http, $location) {
    ...
    $scope.editBarang = function (KODE, NAMA_BARANG, ID_JENIS, JUMLAH_UNIT,
        ...
        $scope.kode = KODE;
        $scope.nama_barang = deskripsi(NAMA_BARANG);
        $scope.jenis_barang = {
            id: ID_JENIS
        };
        $scope.jumlah_unit = deskripsi(JUMLAH_UNIT);
        $scope.kondisi = deskripsi(KONDISI);
        $scope.harga_beli = deskripsi(HARGA_BELI);
        $scope.keterangan = deskripsi(KETERANGAN);
    }
});
...
app.controller('testEnkripsi', function ($scope, $http) {
    ...
    $scope.tampilEnkripsi = function (plaintext) {
        ...
        for (index in plaintext) {
            let code = plaintext[index].charCodeAt(0);
            code = code + 3;
            chars.push(String.fromCharCode(code));
            console.log(plaintext[index] + " => " + chars[index]);
        }
        ...
        $scope.encryptext = chars;
    }

    // fungsi untuk menampilkan hasil deskripsi berdasarkna encryptext
    $scope.tampilDeskripsi = function (encryptext) {
